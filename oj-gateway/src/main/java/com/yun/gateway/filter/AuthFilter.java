...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        Claims claims;
        try {
            claims = JwtUtils.parseToken(token, secret); //获取令牌中信息 解析payload中信息 payload中存着用户的一些标识性的信息
            if (claims == null) {
    ...
    @Override
    public int getOrder() {
        return -200; // 未来可能有多个过滤器 这里的值越小 过滤器就越优先去执行
    }
}
